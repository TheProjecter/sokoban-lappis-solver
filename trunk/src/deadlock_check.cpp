...
bool is_freeze_deadlock(soko_node *node, int *num_neighbors,
	...
	int further_cell_rel=abs_to_rel_table[further_cell_y*board_width+further_cell_x];

	if(num_neighbors[box_rel]==4) {
		if( (node->box_pos[further_cell_rel/int_bits] & (1<<(further_cell_rel%int_bits))) == 0)
			return false;
        
		int near_box_1=abs_to_rel_table[(box_y+(horiz_vert?1:0))*board_width+box_x+(horiz_vert?0:1)];
		int near_box_2=abs_to_rel_table[(box_y-(horiz_vert?1:0))*board_width+box_x-(horiz_vert?0:1)];
		int near_further_1=abs_to_rel_table[(further_cell_y+(horiz_vert?1:0))*board_width+further_cell_x+(horiz_vert?0:1)];
		int near_further_2=abs_to_rel_table[(further_cell_y-(horiz_vert?1:0))*board_width+further_cell_x-(horiz_vert?0:1)];
				
/*
cout << "box_y: " << box_y << endl;
cout << "box_x: " << box_x << endl;
cout << "further_cell_y: " << further_cell_y << endl;
cout << "further_cell_x: " << further_cell_x << endl;

cout << "near_box_1_y: " << (box_y+(horiz_vert?1:0)) << endl;
cout << "near_box_1_x: " << (box_x+(horiz_vert?0:1)) << endl;
cout << "near_further_1_y: " << (further_cell_y+(horiz_vert?1:0)) << endl;
cout << "near_further_1_x: " << (further_cell_x+(horiz_vert?0:1)) << endl;

cout << "near_box_2_y: " << (box_y-(horiz_vert?1:0)) << endl;
cout << "near_box_2_x: " << (box_x-(horiz_vert?0:1)) << endl;
cout << "near_further_2_y: " << (further_cell_y-(horiz_vert?1:0)) << endl;
cout << "near_further_2_x: " << (further_cell_x-(horiz_vert?0:1)) << endl;

cout << "near_box_1 is a box: " << (node->box_pos[near_box_1/int_bits] & (1<<(near_box_1%int_bits)) ) << endl;
cout << "near_further_1 is a wall: " << (near_further_1 == -1) << endl;
cout << "near_further_1 is a box: " << ( node->box_pos[near_further_2/int_bits] & (1<<(near_further_2%int_bits))) << endl;
cout << "near_box_2 is a box: " << (node->box_pos[near_box_2/int_bits] & (1<<(near_box_2%int_bits)) ) << endl;
cout << "near_further_2 is a wall: " << (near_further_2 == -1) << endl;
cout << "near_further_2 is a box: " << ( node->box_pos[near_further_2/int_bits] & (1<<(near_further_2%int_bits))) << endl;
*/

		if( ( (node->box_pos[near_box_1/int_bits] & (1<<(near_box_1%int_bits)) ) &&
				(near_further_1 == -1 || ( node->box_pos[near_further_1/int_bits] & (1<<(near_further_1%int_bits)))) )
			|| ( (node->box_pos[near_box_2/int_bits] & (1<<(near_box_2%int_bits)) ) &&
				(near_further_2 == -1 || ( node->box_pos[near_further_2/int_bits] & (1<<(near_further_2%int_bits)))) ) )
			return true;
		...
		int near_further_1=abs_to_rel_table[(further_cell_y+(horiz_vert?1:0))*board_width+further_cell_x+(horiz_vert?0:1)];
		int near_further_2=abs_to_rel_table[(further_cell_y-(horiz_vert?1:0))*board_width+further_cell_x-(horiz_vert?0:1)];
		int near_man_1=abs_to_rel_table[(man_y+(horiz_vert?1:0))*board_width+man_x+(horiz_vert?0:1)];
		int near_man_2=abs_to_rel_table[(man_y-(horiz_vert?1:0))*board_width+man_x-(horiz_vert?0:1)];
		...
		if(further_cell_rel == -1) {
			...
			if( ((node->box_pos[near_box_1/int_bits] & (1<<(near_box_1%int_bits)))
				&& (near_further_1==-1
					|| near_man_1==-1
					|| (node->box_pos[near_further_1/int_bits] & (1<<(near_further_1%int_bits)))))
				|| ((node->box_pos[near_box_2/int_bits] & (1<<(near_box_2%int_bits)))
				&& (near_further_2==-1
					|| near_man_2==-1
					|| (node->box_pos[near_further_2/int_bits] & (1<<(near_further_2%int_bits))))))
				return true;
		} else {
			if( (node->box_pos[further_cell_rel/int_bits] & (1<<(further_cell_rel%int_bits)))
				&& ((near_box_1==-1
					&& (near_further_1==-1
						|| near_further_2==-1
						|| (node->box_pos[near_further_1/int_bits] & (1<<(near_further_1%int_bits)))))
		...
		if(further_cell_rel != -1 && (node->box_pos[further_cell_rel/int_bits] & (1<<(further_cell_rel%int_bits))))
			return true;
	}
	return false;
}
